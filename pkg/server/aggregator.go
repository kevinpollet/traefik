...
// FIXME Using consul and catalog does not mean necessarily using namespaces
func checkConfiguration(providerName string, configuration dynamic.Configuration) dynamic.Configuration {
	conf := dynamic.Configuration{
		HTTP: &dynamic.HTTPConfiguration{
			Routers:           make(map[string]*dynamic.Router),
			Middlewares:       make(map[string]*dynamic.Middleware),
