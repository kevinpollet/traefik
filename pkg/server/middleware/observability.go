...
func (o *ObservabilityMgr) BuildEPChain(ctx context.Context, entryPointName string, serviceName string, observabilityConfig *dynamic.RouterObservabilityConfig) alice.Chain {
	...
	if (o.tracer != nil && o.ShouldAddTracing(serviceName, observabilityConfig)) || (o.metricsRegistry != nil && o.metricsRegistry.IsEpEnabled() && o.ShouldAddMetrics(serviceName, observabilityConfig)) {
		chain = chain.Append(observability.WrapEntryPointHandler(ctx, o.tracer, o.semConvMetricRegistry, entryPointName))
	}

	if o.metricsRegistry != nil && o.metricsRegistry.IsSvcEnabled() && !o.ShouldAddMetrics(serviceName, observabilityConfig) {
		chain = chain.Append(func(next http.Handler) (http.Handler, error) {
			return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
				next.ServeHTTP(rw, req.WithContext(context.WithValue(req.Context(), mmetrics.DisableServiceMetricsKey, true)))
			}), nil
		})
	}
